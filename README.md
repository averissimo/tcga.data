...
if (file.exists('cache.RData')) {
  ...
  tcga.data <- TCGA2STAT::getTCGA(disease = "BRCA", data.type = "RNASeq2", type = "RPKM", clinical = TRUE)
  save(tcga.data, file = 'cache.RData')
}
```
...
tissue.barcode <- list()
tissue.barcode$all <- gsub(tcga.barcode.pattern, '\\1', colnames(tcga.data$dat))
#
# getting all tisues
tissue <- list()
tissue$all <- tcga.data$dat
#
# clinical data
clinical <- list()
clinical$all <- tcga.data$clinical
#
#
...
tissue.type  <- as.numeric(gsub(tcga.barcode.pattern,'\\2', colnames(tcga.data$dat)))
#
