...
if (file.exists(cached.file)) {
  ...
  #
  flog.info('Preparing query for RNASeq data...')
  query$rnaseq <- GDCquery(project = project,
                           data.category = "Transcriptome Profiling",
                           data.type     = "Gene Expression Quantification",
                           workflow.type = "HTSeq - FPKM")
  #
  flog.info('Downloading RNASeq data...')
...
#
#
levels(clinical$all$vital_status) <- levels(clinical$all$vital_status) %>% {replace(., . == '', NA)}
levels(follow.up$vital_status)    <- levels(follow.up$vital_status) %>% {replace(., . == '', NA)}

updated.surv           <- update.survival.from.followup(clinical$all, follow.up) %>% as.data.frame
rownames(updated.surv) <- updated.surv$bcr_patient_barcode

clinical$all$surv_event_time <- updated.surv[clinical$all$bcr_patient_barcode, 'surv_event_time']
clinical$all$vital_status    <- updated.surv[clinical$all$bcr_patient_barcode, 'vital_status']
