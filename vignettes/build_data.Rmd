...
fpkm.per.tissue.barcode <- getParticipantCode(gdc$rnaseq.fpkm@colData$barcode)
#
# getting all tisues
fpkm.per.tissue           <- as.matrix(gdc$rnaseq.fpkm@assays$data[["HTSeq - FPKM"]])
colnames(fpkm.per.tissue) <- gdc$rnaseq.fpkm@colData$barcode
rownames(fpkm.per.tissue) <- gdc$rnaseq.fpkm@rowRanges$ensembl_gene_id

# Counts
counts.per.tissue           <- as.matrix(gdc$rnaseq.counts@assays$data[["HTSeq - Counts"]])
colnames(counts.per.tissue) <- gdc$rnaseq.counts@colData$barcode
rownames(counts.per.tissue) <- gdc$rnaseq.counts@rowRanges$ensembl_gene_id

#
gene.ranges <- gdc$rnaseq.fpkm@rowRanges
#
# clinical data
...
multiAssay <- build.assay(clinical.custom      = clinical, 
                          ...
                          rnaseq.fpkm.custom   = fpkm.per.tissue,
                          rnaseq.counts.custom = counts.per.tissue)
```
...
rmarkdown::render('build_data.Rmd', output_file = 'README.md', output_dir = '..', params = list(project = "TCGA-SKCM"))
