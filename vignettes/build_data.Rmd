...
if (file.exists(cached.file)) {
  ...
  GDCdownload(query$biospecimen, method = 'api', files.per.chunk = 100)
  #
...
#
# clinical data
if (any(duplicated(gdc$clinical$bcr_patient_barcode))) {
  dup.barcodes <- gdc$clinical$bcr_patient_barcode %>% { .[duplicated(.) %>% which] }
  .Last.value <- sapply(dup.barcodes, function(ix) { 
    sub.table <- filter(gdc$clinical, bcr_patient_barcode == ix) 
    should.break <- apply(sub.table, 2, function(ix.col) { table(ix.col) }) %>%
      sapply(function(ix.res) { length(ix.res) %in% c(0,1)}) %>%
      all
    if (!should.break) {
      stop('There are non-equal rows with same bcr_patient_barcode')
    }
  })
  gdc$clinical <- gdc$clinical[!duplicated(gdc$clinical$bcr_patient_barcode),]
}
