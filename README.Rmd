...
tissue.barcode <- list()
tissue.barcode$all <- gsub(tcga.barcode.pattern, '\\1', gdc$rnaseq@colData$barcode)
#
# getting all tisues
tissue <- list()
tissue$all <- gdc$rnaseq@assays$data[['HTSeq - FPKM']]
...
tissue.type  <- as.numeric(gsub(tcga.barcode.pattern,'\\2', colnames(tissue$all)))
#
